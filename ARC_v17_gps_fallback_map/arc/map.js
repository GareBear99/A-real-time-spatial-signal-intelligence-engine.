...
export function renderStructures(structures){
  ...
  structures.forEach(s => {
    ...
    const poly = L.polygon(s.polygon, {
      color: isSelected ? '#67f6ff' : '#79c1ff',
      weight: isSelected ? 3 : 2,
      fillColor: isSelected ? '#67f6ff' : '#2670a8',
...
export function renderSelection(){
  ...
  L.polygon(s.polygon, { color:'#a6ff68', weight:3, fillOpacity:0.06, dashArray:'5, 5' }).addTo(state.layers.selection);
...
export function renderSensors(){
  ...
  for(const s of state.sensors){
    L.circleMarker([s.lat, s.lng], {
      ...
      color: s.type === 'center-anchor' ? '#a6ff68' : '#ffd166',
      weight: 2,
      fillOpacity: 0.7
...
export function renderDevice(device){
  ...
  if(!device) return;
  if(device.truth){ L.circleMarker([device.truth.lat, device.truth.lng], { radius:6, color:'#a6ff68', fillOpacity:0.85 }).addTo(state.layers.truth).bindTooltip('Ground Truth'); }
  if(device.estimated){ L.circleMarker([device.estimated.lat, device.estimated.lng], { radius:6, color:'#67f6ff', fillOpacity:0.85 }).addTo(state.layers.estimate).bindTooltip('Estimated'); }
  if(device.truthPath.length > 1){ L.polyline(device.truthPath.map(p => [p.lat, p.lng]), { color:'#a6ff68', weight:2, opacity:0.55 }).addTo(state.layers.paths); }
  if(device.estimatePath.length > 1){ L.polyline(device.estimatePath.map(p => [p.lat, p.lng]), { color:'#67f6ff', weight:2, opacity:0.75 }).addTo(state.layers.paths); }
}
