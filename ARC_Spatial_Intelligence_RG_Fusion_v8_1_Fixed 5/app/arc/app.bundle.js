...
function renderTiles(){ if(!state.map.tiles) return; const zInt = Math.max(0, Math.floor(state.map.zoom)); const zScale = Math.pow(2, state.map.zoom - zInt); const worldCenter = mercatorWorld(state.map.center, zInt); const scaledW = state.map.width / zScale, scaledH = state.map.height / zScale; const startX = worldCenter.x - scaledW/2, startY = worldCenter.y - scaledH/2; const endX = worldCenter.x + scaledW/2, endY = worldCenter.y + scaledH/2; const overscan = 1; const minTileX = Math.floor(startX/TILE_SIZE)-overscan, maxTileX = Math.floor(endX/TILE_SIZE)+overscan; const minTileY = Math.floor(startY/TILE_SIZE)-overscan, maxTileY = Math.floor(endY/TILE_SIZE)+overscan; const provider = PROVIDERS[state.map.provider] || PROVIDERS.topo; const limit = Math.max(1, Math.pow(2,zInt));
  let html = '';
  for(let tx=minTileX; tx<=maxTileX; tx++){
    for(let ty=minTileY; ty<=maxTileY; ty++){
      if(ty < 0 || ty >= limit) continue;
      const wrapX = ((tx % limit) + limit) % limit; const left = (tx*TILE_SIZE - startX) * zScale, top = (ty*TILE_SIZE - startY) * zScale; const size = TILE_SIZE * zScale;
      const url = provider.url.replace('{z}', zInt).replace('{x}', wrapX).replace('{y}', ty);
      const fog = Math.max(0, Math.min(0.28, Math.abs(ty-(minTileY+maxTileY)/2)*0.018));
      html += `<img src="${url}" style="position:absolute;left:${left.toFixed(2)}px;top:${top.toFixed(2)}px;width:${size.toFixed(2)}px;height:${size.toFixed(2)}px;object-fit:cover;opacity:${(1-fog).toFixed(3)};transform:translateZ(0) scale(1);">`;
    }
  }
  state.map.tiles.innerHTML = html;
}
function renderSvg(){ if(!state.map.svg) return; const structure = state.selectedStructure; const device = state.devices.phone1 || ensureDevice(); const lines = []; const z = state.map.zoom;
  ...
  const text = (p, label, size=11, opacity=0.82) => `<text x="${(p.x+8).toFixed(1)}" y="${(p.y-8).toFixed(1)}" fill="var(--fg)" fill-opacity="${opacity}" font-size="${size}">${escapeHtml(label)}</text>`;
  const depthFade = Math.max(0.18, Math.min(1, (z-14.5)/2.5));
  if(structure){ const pts = structure.polygon.map(([lat,lng]) => project({lat,lng})); lines.push(polyline([...pts, pts[0]], `fill="rgba(122,60,255,${(0.06*depthFade).toFixed(3)})" stroke="rgba(177,140,255,${(0.75*depthFade).toFixed(3)})" stroke-width="${(1.4 + depthFade*1.8).toFixed(2)}" stroke-linejoin="round" data-structure-id="${escapeHtml(structure.id)}" style="cursor:pointer;filter:drop-shadow(0 0 10px rgba(122,60,255,.18))"`)); if(z >= 16.2){ const c = project(structure.center); lines.push(text(c, structure.address, z >= 17.2 ? 11.5 : 10.5, 0.9)); } }
  for(const sensor of state.sensors){ const p = project(sensor); const baseR = sensor.type==='center-anchor' ? 5.5 : 4.25; const ringR = sensor.type==='center-anchor' ? 26 : 18; if(state.flags.showRadius) lines.push(circle(p, ringR + (z-15)*3.5, 'fill="rgba(122,60,255,0.03)" stroke="rgba(122,60,255,0.14)" stroke-width="1"')); lines.push(circle(p, baseR + Math.max(0,z-16)*0.7, `fill="${sensor.type==='center-anchor' ? '#7A3CFF' : '#B18CFF'}" stroke="rgba(255,255,255,.92)" stroke-width="1.8"`)); }
  if(device.candidateCloud?.length){ for(const cp of device.candidateCloud){ const p = project(cp); lines.push(circle(p, 1.5 + Math.max(0, z-16)*0.12, 'fill="rgba(122,60,255,0.16)" stroke="none"')); } }
  if(device.truthPath.length>1){ lines.push(polyline(device.truthPath.map(project), 'fill="none" stroke="rgba(69,210,140,.50)" stroke-width="2" opacity="0.55" stroke-linecap="round" stroke-linejoin="round"')); }
  if(device.estimatePath.length>1){ lines.push(polyline(device.estimatePath.map(project), 'fill="none" stroke="rgba(154,113,255,.90)" stroke-width="2.6" opacity="0.88" stroke-linecap="round" stroke-linejoin="round"')); }
  if(device.truth && state.flags.showMarker){ const p = project(device.truth); lines.push(circle(p, 4.5 + Math.max(0,z-16)*0.35, 'fill="#2f9e62" stroke="rgba(255,255,255,.96)" stroke-width="1.8"')); }
  if(device.estimated && state.flags.showMarker){ const p = project(device.estimated); const rad = Math.max(14, 44*(1-(device.confidence||0))+14); if(state.flags.showRadius){ lines.push(circle(p, rad, 'fill="rgba(122,60,255,0.08)" stroke="rgba(122,60,255,0.22)" stroke-width="1.8"')); lines.push(circle(p, rad*0.56, 'fill="rgba(122,60,255,0.04)" stroke="rgba(177,140,255,0.12)" stroke-width="1"')); } lines.push(circle(p, 7.5, 'fill="#7A3CFF" stroke="rgba(255,255,255,.96)" stroke-width="2"')); lines.push(circle(p, 12.5, 'fill="none" stroke="rgba(177,140,255,.55)" stroke-width="1.6"')); lines.push(`<path d="M ${p.x.toFixed(2)} ${(p.y-19).toFixed(2)} L ${(p.x+8).toFixed(2)} ${(p.y-7).toFixed(2)} L ${(p.x-8).toFixed(2)} ${(p.y-7).toFixed(2)} Z" fill="rgba(255,255,255,.82)" opacity="0.9"/>`); lines.push(`<text x="${(p.x+14).toFixed(1)}" y="${(p.y+4).toFixed(1)}" fill="var(--fg)" font-size="10.5" fill-opacity="0.92">F${device.estimatedFloor||1}</text>`); }
  if(structure && state.advanced.geofenceRadius>0){ const c = project(structure.center); lines.push(circle(c, Math.max(22, state.advanced.geofenceRadius*2.4), 'fill="none" stroke="rgba(245,180,0,0.48)" stroke-dasharray="6 5" stroke-width="1.8"')); }
...
function renderRF(){ if(!state.map.rf) return; const ctx = state.map.rf.getContext('2d'); if(!ctx) return; ctx.clearRect(0,0,state.map.width,state.map.height); const w=state.map.width,h=state.map.height; const cx=w/2, cy=h/2; const vignette = ctx.createRadialGradient(cx, cy, Math.min(w,h)*0.12, cx, cy, Math.max(w,h)*0.72); vignette.addColorStop(0,'rgba(255,255,255,0)'); vignette.addColorStop(0.55,'rgba(122,60,255,0.035)'); vignette.addColorStop(1,'rgba(0,0,0,0.18)'); ctx.fillStyle=vignette; ctx.fillRect(0,0,w,h); const horizon=ctx.createLinearGradient(0,0,0,h); horizon.addColorStop(0,'rgba(255,255,255,0.06)'); horizon.addColorStop(0.32,'rgba(255,255,255,0.0)'); horizon.addColorStop(1,'rgba(0,0,0,0.10)'); ctx.fillStyle=horizon; ctx.fillRect(0,0,w,h); const device = state.devices.phone1 || ensureDevice(); if(!device.estimated) return; const center = project(device.estimated); const pulse = (Date.now()/700)%1; const radius = 105 + pulse*18; const gradient = ctx.createRadialGradient(center.x, center.y, 0, center.x, center.y, radius); gradient.addColorStop(0, 'rgba(122,60,255,0.22)'); gradient.addColorStop(0.45, 'rgba(122,60,255,0.08)'); gradient.addColorStop(1, 'rgba(122,60,255,0.0)'); ctx.fillStyle = gradient; ctx.beginPath(); ctx.arc(center.x, center.y, radius, 0, Math.PI*2); ctx.fill(); ctx.strokeStyle='rgba(177,140,255,0.30)'; ctx.lineWidth=1.2; ctx.beginPath(); ctx.arc(center.x, center.y, 26 + pulse*8, 0, Math.PI*2); ctx.stroke(); ctx.strokeStyle='rgba(177,140,255,0.18)'; ctx.beginPath(); ctx.arc(center.x, center.y, 52 + pulse*12, 0, Math.PI*2); ctx.stroke(); }
...
window.__ARC = {state, renderMap, setCenterZoom, fitPoints, applyIngestPoints, parseIngestPayload, distanceMeters, centroid, boundsFromPolygon, downloadJson, project, lockStructure, clearLock, searchStructures};
